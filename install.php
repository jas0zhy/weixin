...
if($action == 'env') {
	...
	if(version_compare(PHP_VERSION, '5.3.0') == -1) {
		...
		$ret['php']['version']['remark'] = 'PHP版本必须为 5.3.0 以上. <a href="http://shop115222832.taobao.com">详情</a>';
	}
	...
	if($ret['php']['pdo']['ok']) {
		...
		if(!$ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['remark'] = '您的PHP环境虽然不支持 mysql_connect, 但已经支持了PDO, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
		}
	} else {
		if($ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['pdo']['class'] = 'warning';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 但支持 mysql_connect, 这样系统虽然可以运行, 但还是建议你开启PDO以提升程序性能和系统稳定性. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
		} else {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['pdo']['class'] = 'danger';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 也不支持 mysql_connect, 系统无法正常运行. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
	...
	if($ret['php']['curl']['ok']) {
		...
		if(!$ret['php']['fopen']['ok']) {
			$ret['php']['curl']['remark'] = '您的PHP环境虽然不支持 allow_url_fopen, 但已经支持了cURL, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
		}
	} else {
		if($ret['php']['fopen']['ok']) {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['curl']['class'] = 'warning';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 但支持 allow_url_fopen, 这样系统虽然可以运行, 但还是建议你开启cURL以提升程序性能和系统稳定性. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
		} else {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['curl']['class'] = 'danger';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 也不支持 allow_url_fopen, 系统无法正常运行. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
	...
	if($ret['php']['ssl']['ok']) {
		...
		$ret['php']['ssl']['remark'] = '没有启用OpenSSL, 将无法访问公众平台的接口, 系统无法正常运行. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
	}
	...
	if($ret['php']['gd']['ok']) {
		...
		$ret['php']['gd']['remark'] = '没有启用GD, 将无法正常上传和压缩图片, 系统无法正常运行. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
	}
	...
	if($ret['php']['dom']['ok']) {
		...
		$ret['php']['dom']['remark'] = '没有启用DOMDocument, 将无法正常安装使用模块, 系统无法正常运行. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
	}
	...
	if($ret['php']['session']['ok'] == 0 || strtolower($ret['php']['session']['ok']) == 'off') {
		...
		$ret['php']['session']['remark'] = '系统session.auto_start开启, 将无法正常注册会员, 系统无法正常运行. <a target="_blank" href="http://shop115222832.taobao.com">详情</a>';
	}
...
if($action == 'db') {
	if($ispost) {
		...
		if(empty($error)) {
			...
			$dbfile = IA_ROOT . '/data/wshcms.php';
			...
			if($_POST['type'] == 'remote') {
				...
				file_put_contents(IA_ROOT . '/data/wshcms.php', base64_decode($ins['schemas']));
...
VER;
			...
			mysql_query("INSERT INTO {$db['prefix']}users (uid, username, password, salt, joindate) VALUES('{$uid}', '{$user['username']}', '{$password}', '{$salt}', '" . time() . "')");
			local_mkdirs(IA_ROOT . '/data');
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - 人人店分销人人分销商城 - 公众平台自助开源引擎</title>
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				<h3>版权所有 (c)2015，人人店分销人人分销商城 保留所有权利。 </h3>
				<p>
                感谢您选择人人店分销人人分销商城 - 微信三级分销企业版管理系统（以下简称RENREN，RENREN基于 PHP + MySQL的技术开发，全部源码开放。 <br />
					为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：
				</p>
				<p>
					<strong>一、本授权协议适用且仅适用于势力人人分销商城系统任何版本，势力人人分销商城官方对本授权协议的最终解释权。</strong>
				</p>
				<p>
					<strong>二、协议许可的权利 </strong>
					<ol>
						<li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。</li>
						<li>您可以在协议规定的约束和限制范围内修改人人源代码或界面风格以适应您的网站要求。</li>
				...
				<p>
					...
					<ol>
						...
						<li>未经官方许可，禁止在人人的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</li>
				...
				<p>
					...
					<ol>
						...
						<li>电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装  RENREN，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</li>
...
function tpl_install_env($ret = array()) {
	...
	echo <<<EOF
		...
		<div class="alert alert-info">系统要求人人店分销人人分销商城整个安装目录必须可写, 才能使用人人店分销人人分销商城所有功能。</div>
		<div class="panel panel-default">
...
function tpl_install_finish() {
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“人人店分销人人分销商城 - 微信三级分销企业版管理系统，您现在可以: <a target="_blank" class="btn btn-success" href="./web/index.php">访问网站首页</a>
	</div>
