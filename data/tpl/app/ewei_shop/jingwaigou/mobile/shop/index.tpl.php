...
<link rel="stylesheet" type="text/css" href="../addons/ewei_shop/template/mobile/jingwaigou/new/index.css">
<style type="text/css">
    /*.调用index.css，下面css失效
    ...
    .banner {height:200px; width:100%; background:#fff;overflow:hidden;position:relative;}
    .banner .main_image{height:200px;position:relative;top:0;left:0;}
    .banner .main_image ul{height:200px;position:absolute;top:0;left:0;}
    .banner .main_image li{float:left;height:200px;list-style:none;padding:0;margin-top:-20px;}
    .banner .main_image li img{display:block;width:100%; height:100%;}
    .banner .main_image li a{display:block;width:100%;height:200px}
...
<script id='tpl_index' type='text/html'>
    ...
    <%/if%>
    <?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('new/baozheng', TEMPLATE_INCLUDEPATH)) : (include template('new/baozheng', TEMPLATE_INCLUDEPATH));?>
    <?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('new/globle-floor', TEMPLATE_INCLUDEPATH)) : (include template('new/globle-floor', TEMPLATE_INCLUDEPATH));?>
    <!--
...
<script id='tpl_goods_list' type='text/html'>
    ...
    <div class="good" data-goodsid='<%g.id%>'>
        ...
        <div class="good-item">
          ...
          <p class='sale-price'><span>￥</span><%g.marketprice%> </p>
          <p class='price'>国内参考价<%if g.productprice>0 && g.marketprice!=g.productprice%><span>￥<%g.productprice%></span><%/if%></p>
        </div>
    </div>
    <%/each%>
