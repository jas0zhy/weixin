		...
		<form method="post" action="<?php  echo url('mc/bond/consume');?>" role="form" id="form1" class="form-horizontal form">
			<div class="form-group">
				<div class="col-xs-12">
					<select name="store_id" id="store_id" class="form-control">
						...
						<option value="<?php  echo $store['id'];?>"><?php  echo $store['business_name'];?></option>
						<?php  } } ?>
					</select>
				</div>
			</div>
			<div class="form-group">
				<div class="col-xs-12">
					...
					<div class="help-block"><strong class="text-danger"><?php  echo $tips;?></strong></div>
				</div>
			</div>
...
<script>
	require(['util'], function(u){
		$(':text[name="credit"]').keyup(function(){
			...
			var price = credit;
			<?php  if($setting['discount_type'] > 0 && !empty($mine_discount)) { ?>
				var discount = '<?php  echo json_encode($mine_discount);?>';
				discount = $.parseJSON(discount);
				var type = "<?php  echo $setting['discount_type'];?>";
		...
		$('#form1').submit(function(){
			...
			var member_credit = parseFloat("<?php  echo $_W['member']['credit2'];?>");
