...
<script type="text/javascript">
	require(['angular', 'jquery.ui', 'underscore'], function(angular, $, _){
		angular.module('app', []).controller('menuDesigner', function($scope, $http){
			...
			$scope.select_mediaid = function(){
				var option = {
						'ignore' : {
							'wxcard' : true
						}
					};
				if($scope.activeMenu.type == 'view_limited') {
					option.ignore = {
						'image' : true,
						'voice' : true,
						'video' : true,
				...
				util.material(function(material){
					$scope.activeMenu.media_id = material.media_id;
					$scope.$digest();
				}, option);
			};
		});
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<label class="radio-inline">
								<input type="radio" name="ipt" ng-model="activeMenu.type" value="media_id"> 回复素材
							</label>
							<label class="radio-inline">
								<input type="radio" name="ipt" ng-model="activeMenu.type" value="view_limited"> 跳转图文
							</label>
							<div <?php  if($_W['account']['level'] == '1') { ?>style="display:none;"<?php  } ?>ng-show="activeMenu.type == 'url';">
								<hr />
							...
							<div <?php  if($_W['account']['level'] != '1') { ?>ng-show="activeMenu.type == 'media_id' || activeMenu.type == 'view_limited';"<?php  } ?>>
								<hr />
								<div class="input-group">
									<input class="form-control" id="ipt-url" type="text" ng-model="activeMenu.media_id" />
									<div class="input-group-btn">
										<button class="btn btn-primary" id="media_id" ng-click="select_mediaid()"><i class="fa fa-external-link"></i> 选择素材</button>
									</div>
								</div>
								<span class="help-block">公众平台的素材id</span>
							</div>
							<div ng-show="activeMenu.type != 'url' && activeMenu.type != 'media_id' && activeMenu.type != 'view_limited'" style="position:relative">
								<hr />
								<div class="input-group">
