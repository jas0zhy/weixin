...
class weisrc_dishModuleSite extends WeModuleSite
{
    ...
    public $_auth2_key = 'c2hvcHBpbmdvLmNu';
    public $_lat = '';
    public $_lng = '';
            ...
            if ($this->_accountlevel == 4) {
                ...
                if (!empty($setting)) {
                    if (!empty($setting['tpluser'])) {
                        $this->sendAdminQueueNotice($oid, $setting['tpluser'], $setting);
                    }
    ...
    public function resetn(){$this->table_queue_order = "we" . "isrc_di" . "sh_que" . "ue_order";$this->table_print_order = "weis" . "rc_di" . "sh_print_order";$this->table_mealtime = 'weis' . 'rc_di' . 'sh_me' . 'altime';}
...
//                $this->sendOrderSms($order);
                ...
                if (!empty($setting)) {
                    ...
                    if ($setting['is_notice'] == 1) {
                        if (!empty($setting['tpluser'])) {
                            $this->sendAdminOrderNotice($orderid, $setting['tpluser'], $setting);
                        }
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
        ...
        if (checksubmit('submit')) {
            ...
            if ($data['email_enable'] == 1) {
                ...
                $mail_config['subject'] = '淘宝搜索 @百川信科提醒';
...
//                        exit('餐桌类型不存在！');
    ...
    public function setOrderCredit($orderid, $add = true) {
        ...
        if (!empty($credit)) {
            ...
            if (!empty($fans)) {
                $uid = intval($fans['uid']);
                $remark = $add == true ? '淘宝搜索 @百川信科积分奖励 订单ID:' . $orderid : '淘宝搜索 @百川信科积分扣除 订单ID:' . $orderid;
                $log = array();
                $log[0] = $uid;
                $log[1] = $remark;
    ...
    */
    ...
    public function doWebSetRule()
    {
        ...
        if (empty($rule)) {
            header('Location: ' . $_W['siteroot'] . create_url('rule/post', array('module' => 'weisrc_dish', 'name' => '淘宝搜索 @百川信科')));
            exit;
        } else {
    ...
    public $copyright = '111001 110000 110111 110001 110001 110101 110000 110100 110101';
    ...
    function doMobileSv(){echo $this->copyright;}
